...
public class AppManagerActivity extends Activity {
    ...
    private List<AppBean> userApks = new ArrayList<AppBean>();//用户APK容器
    private List<AppBean> sysApks = new ArrayList<AppBean>();//系统APK容器
...
//        // 启动分享GUI
//        oks.show(this);
        Toast.makeText(getApplicationContext(), "分享功能暂未实现", Toast.LENGTH_SHORT).show();
    }
    ...
    protected void removeApk() {
		 ...
		 */
            ...
            intent.setData(Uri.parse("package:" + clickBean.getPackName()));
            startActivity(intent);
        } else {//2.系统APK(默认删除不掉，root刷机，赋予root权限，才可以删除)
            try {
                //判断是否root刷机
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case LOADING://加载数据
                    ...
                    tv_listview_lable.setVisibility(View.GONE);//隐藏列表标签
                    break;
                case FINISH://数据加载完成
                    pb_loading.setVisibility(View.GONE);//隐藏进度条
                    lv_datas.setVisibility(View.VISIBLE);//显示ListView
                    tv_listview_lable.setVisibility(View.VISIBLE);//显示列表标签
                    // 设置内存剩余大小
                    tv_sdAvail.setText("SD卡可用空间:"
                            + Formatter.formatFileSize(getApplicationContext(),
                            sdAvail));
                    tv_romAvail.setText("ROM可用空间:"
                            + Formatter.formatFileSize(getApplicationContext(), romAvail));
                    // 初始化标签
                    tv_listview_lable.setText("用户软件(" + userApks.size() + ")");
    ...
    private class MyAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (position == 0) {//用户APK的标签
                ...
                return tv_userTable;
            } else if (position == userApks.size() + 1) {//用户APK的标签
                TextView tv_userTable = new TextView(getApplicationContext());
                ...
                if (convertView != null && convertView instanceof RelativeLayout) {
                    ...
                    convertView.setTag(holder);//将holder存储在convertView中
                }
                ...
                if (bean.isSD()) {
                    holder.tv_location.setText("SD卡存储");// 设置存储位置
                } else {
              ...
              /* 获取所有APK数据*/
                ...
                for (AppBean appBean : datas) {
                    if (appBean.isSystem()) {
                        ...
                        userApks.add(appBean);//用户APK
                    }
                }
                sdAvail = AppManagerEngine.getSDAvail(getApplicationContext());//获取SD卡中的APK数据
                romAvail = AppManagerEngine.getRomAvail(getApplicationContext());//获取ROM中的APK数据
               /*数据处理完毕，发送消息*/
    ...
    private void initView() {
        ...
        adapter = new MyAdapter();//ListView适配器
        lv_datas.setAdapter(adapter);//加载适配器
        pm = getPackageManager();
    }

    @Override
    protected void onDestroy() {
        unregisterReceiver(receiver);//取消删除APK的广播
        super.onDestroy();
    }
}
