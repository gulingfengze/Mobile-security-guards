...
public class CacheActivity extends AppCompatActivity {
    ...
    private static final int SCANDING = 1;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case SCANDING://正在扫描
                    ...
                    ll_mess.setVisibility(View.GONE);// 隐藏LinearLayout组件
                    tv_nodata.setVisibility(View.GONE);// 隐藏没有数据的组件
     ...
     * 搜索所有app 查询缓存信息
     */
    private void scanAllapp() {//耗时操作
        new Thread(new Runnable() {
            @Override
            public void run() {
                handler.obtainMessage(SCANDING).sendToTarget();// 发送一个显示进度的消息

                /*遍历所有安装的app ，看是否有缓存*/
                allApks = AppManagerEngine.getAllApks(getApplicationContext());
                cacheDatas.clear();// 清空原来的数据
                for (AppBean appBean : allApks) {
//                    Message msg = handler.obtainMessage(SCANDING);
