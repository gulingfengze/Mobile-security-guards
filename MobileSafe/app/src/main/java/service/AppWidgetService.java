...
import receiver.ExampleAppWidgetProvider;
/*清理（桌面小控件形式）*/
    ...
    public void onCreate() {
        ...
        task = new TimerTask() {
            ...
            public void run() {
                ...
                //可用内存
                long availMemSize = TaskManagerEngine.getAvailMemSize(getApplicationContext());
                String availMemStr = Formatter.formatFileSize(getApplicationContext(), availMemSize);
                //给RemoteViews的子组件赋值
                views.setTextViewText(R.id.process_count, "正在运行软件:" + runningNumber);
                views.setTextViewText(R.id.process_memory, "可用内存:" + availMemStr);
                Intent intent = new Intent("mobilesafe.eoe.widget.cleartask");
